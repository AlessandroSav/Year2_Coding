...
#%%
if mod == 'dales':
    dl = DataLoaderDALES(lp,casenr='002')
...
samptend = xr.open_mfdataset(lp+'/samptend.002.nc' )
##########
##########
...
sc0 = axs[0].imshow(u_pfw_pf[z_plot,:,:],
                    aspect='auto',cmap='RdYlBu_r',
                    vmin=np.min(uw_p[z_plot,:,:])*0.02,
                    vmax=-np.min(uw_p[z_plot,:,:])*0.02)

# axs[0].set_yticklabels(np.arange(0,150,(ysize/len(yt))/100))
axs[0].set_ylabel(r'y [m]')
axs[0].title.set_text('Filter scale UW, scale: '+str(round(f_scale/1000, 2))+' km')
pos0 = axs[0].get_position()
cbax0 = fig.add_axes([0.95, pos0.ymin, 0.01, pos0.height])
cb0 = fig.colorbar(sc0, cax=cbax0)
# cb0.ax.set_ylabel(r"u [m/s]", rotation=270, labelpad=15)

sc1 = axs[1].imshow(u_psfw_psf[z_plot,:,:],
                   aspect='auto',cmap='RdYlBu_r',
                   vmin=np.min(uw_p[z_plot,:,:])*0.02,
                   vmax=-np.min(uw_p[z_plot,:,:])*0.02)
axs[1].set_xlabel(r"x[m]")
axs[1].set_ylabel(r'y [m]')
axs[1].title.set_text('Sub-filter scale UW. Height: '+str(int(ztlim[z_plot].values))+' m')
pos1 = axs[1].get_position()
cbax1 = fig.add_axes([0.95, pos1.ymin, 0.01, pos1.height])
cb1 = fig.colorbar(sc1, cax=cbax1)
# cb1.ax.set_ylabel(r"$u$ [m/s]", rotation=270, labelpad=15)
