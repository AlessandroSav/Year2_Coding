...
for path,subdir,files in os.walk(dales_exp_dir):
    if path[-3:] in expnr: 
        for file in glob(os.path.join(path, 'cross_field'+str(z_plot)+'*.nc')):
...
for idx,ii in enumerate(['v','w','vw']):
    ...
    axs[0,idx].set_aspect('equal')
    # axs[0,idx].suptitle(ii)
    axs[0,idx].set_title(r''+snap_time+'\n'+ii+' ${\prime}$ at '+str(z_plot)+'m')
    axs[0,idx].xaxis.set_visible(False) 
    ...
    im_2 = da_cross[ii+'_psf'].sel(time=snap_time).plot(ax=axs[2,idx],\
                               cmap=cm.PiYG_r,vmin=-0.2,vmax=0.2,add_colorbar=False)
    axs[2,idx].set_aspect('equal')
    axs[2,idx].set_title('Sub-filter')
    axs[2,idx].set_xticks([0, 500, 1000, 1500])
...
for idx,ii in enumerate(['uw','vw']):
    ...
    # Net field
    im =(da_cross[ii+'_pf']+da_cross[ii+'_psf']).sel(time=snap_time).plot(ax=axs[idx]\
                               ,cmap=cm.PiYG_r,vmin=vbar[0],vmax=vbar[1],\
                                   add_colorbar=False)
    axs[idx].set_aspect('equal')
    # axs[0,idx].suptitle(ii)
    axs[idx].set_title(r''+snap_time+'\n'+ii+' ${\prime}$ at '+str(z_plot)+'m',fontsize=22)
    axs[idx].set_xticks([500, 1000, 1500])
axs[1].yaxis.set_visible(False)    
axs[0].set_yticks([500, 1000, 1500])
cbar_ax = fig.add_axes([0.99, 0.35, 0.01, 0.3])
...
ii = '2020-02-03T14'
plt.figure()
xr.plot.hist(da_cross['w_p'].sel(time=ii),bins=30,range=[-2,2],alpha=0.5)
xr.plot.hist(da_cross['u_p'].sel(time=ii),bins=35,range=[-2,2],alpha=0.5)
